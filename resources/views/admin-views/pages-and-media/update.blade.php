...
@section('content')
    <div class="content container-fluid">
            ...
            <div class="d-flex gap-10 align-items-center justify-content-between flex-wrap">
                ...
                <div>
                    <a class="btn btn-primary btn-sm" target="_blank"
                        @if ($businessPage['status'] != 1) disabled
                           data-bs-toggle="tooltip"
                           data-bs-title="{{ translate('this_page_is_not_available_on_website_please_turn_on_the_status.') }}"
                    @else
                           href="{{ route('business-page.view', ['slug' => $businessPage['slug'], 'source' => 'admin']) }}" @endif>
                        {{ translate('View_URL') }}
        ...
        <form action="{{ route('admin.pages-and-media.update') }}" method="post" enctype="multipart/form-data" class="form-advance-validation form-advance-inputs-validation form-advance-file-validation non-ajax-form-validate" novalidate="novalidate">
            @csrf
                    ...
                    <div class="card-body d-flex flex-column gap-3 gap-sm-20">
                                ...
                                <div class="col-lg-4">
                                    <label
                                        ...
                                        @if (in_array($businessPage['slug'], ['terms-and-conditions', 'about-us', 'privacy-policy']))
                                            <label class="switcher" for="page-status" data-bs-toggle="tooltip"
                                                ...
                                                <input type="checkbox" class="switcher_input" name="status" value="1"
                                                    id="page-status" checked disabled>
                                                <span class="switcher_control"></span>
                                            </label>
                                        @else
                                            <label class="switcher" for="page-status">
                                                <input type="checkbox" class="switcher_input" name="status" value="1"
                                                    id="page-status" {{ $businessPage['status'] ? 'checked' : '' }}>
                                                <span class="switcher_control"></span>
                                            </label>
                                        @endif
                        ...
                        <div class="p-12 p-sm-20 bg-section rounded">
                            <div class="d-flex flex-column gap-20">
                                ...
                                <div class="upload-file">
                                    ...
                                    <div class="upload-file__wrapper ratio-7-1">
                                    ...
                                    <div class="overlay">
                                        <div class="d-flex gap-10 justify-content-center align-items-center h-100">
                                            ...
                                            @if($businessPage?->banner_full_url['path'])
                                            <button type="button"
                                                class="btn btn-outline-danger icon-btn delete-page-setup-image"
                                                data-id="{{ $businessPage['id'] }}">
                        ...
                        <div class="p-12 p-sm-20 bg-section rounded">
                            <div class="mb-2" id="page-description-form">
                                ...
                                <input class="form-control" name="title" placeholder="{{ translate('type_page_title') }}"
                                    value="{{ $businessPage['title'] }}" data-required-msg="{{ translate('page_title_is_required') }}" required>
                            </div>
                            ...
                            <div class="" id="page-description-form">
                                ...
                                <textarea name="description" id="description-page" style="display:none;" data-required-msg="{{ translate('page_description_is_required') }}" required>{!! $businessPage['description'] !!}</textarea>
                            </div>
                        </div>
        ...
        <form id="admin-page-setup-delete-form-{{ $businessPage['id'] }}" method="POST"
            action="{{ route('admin.pages-and-media.delete.image') }}" method="POST" style="display: none;">
            @csrf
             <input type="hidden" name="id" value="{{ $businessPage['id'] }}">
        </form>
    </div>
...
@push('script')
    ...
    <script src="{{ dynamicAsset(path: 'public/assets/back-end/libs/quill-editor/business-pages-quill-editor-init.js') }}">
    </script>
