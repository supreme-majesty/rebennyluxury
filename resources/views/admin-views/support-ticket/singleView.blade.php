    ...
    <div class="content container-fluid">
        ...
        <div class="card card-chat justify-content-between">
                ...
                @foreach ($supportTicket as $ticket)
                    ...
                    <div class="d-flex align-items-center flex-wrap gap-3">
                        ...
                        <div class="priority d-flex flex-wrap align-items-center gap-3">
                            ...
                            <span
                                class="fw-bold badge text-bg-info badge-info">{{ translate(str_replace('_', ' ', $ticket['priority'])) }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
            <div class="card-body p-3 overflow-y-auto height-220 flex-grow-1 msg_history d-flex flex-column-reverse"
                            ...
                            <div class="sent_msg w-100">
                                <div class="received_withdraw_msg">
                                    ...
                                    @if ($message->admin_message || $message['attachment'] != null)
                                        <span class="time_date fs-12 pt-2 d-flex justify-content-end">
                        ...
                        <div class="incoming_msg">
                            <div class="received_msg p-2">
                                ...
                                @if ($message->customer_message || $message['attachment'] != null)
                                    <span class="time_date fs-12 d-flex justify-content-start pt-2">
                                        {{ $message->created_at->diffForHumans() }}
                                    </span>
                                @endif
                            </div>
                ...
                <div class="incoming_msg  w-75">
                    <div class="received_msg">
                        <div class="received_withdraw_msg">
                            @if ($ticket->description)
                                <div class="d-flex justify-content-start">
                                    <p class="bg-primary text-white rounded px-3 py-2 mb-1 w-max-content">
                                        {{ $ticket->description }}
                                    </p>
                                </div>
                            @endif
                            ...
                            @endif
                            <span class="time_date fs-12 pt-2 d-flex justify-content-start">
                ...
                <div class="type_msg">
                    <div class="input_msg_write">
                        @foreach ($supportTicket as $reply)
                            <form class="needs-validation form-advance-validation form-advance-inputs-validation form-advance-file-validation non-ajax-form-validate" href="{{ route('admin.support-ticket.replay', $reply['id']) }}"
                                    ...
                                    @if (theme_root_path() == 'default')
                                        <label
                                            ...
                                            <input type="file" id="select-media" class="h-100 position-absolute w-100 "
                                                hidden multiple accept="{{ getFileUploadFormats(skip: '.svg,.gif') }},image/*">
                                        </label>
                                    @endif
                                    <div class="w-100">
                                        <textarea class="form-control {{ theme_root_path() == 'default' ? 'ps-5' : '' }}" id="msgInputValue" name="replay" data-required-msg="{{ translate('type_something') }}"
                                            type="text" data-maxlength="189" placeholder="{{ translate('write_your_message_here') }}" required></textarea>
                                        <div class="d-flex justify-content-end">
                                            <span id="charCount" class="fs-12"></span>
                                        </div>
                                    </div>
                                </div>
                                <div class="mt-3 d-flex justify-content-between">
                                    ...
                                    <div>
                                        <button class="aSend btn btn-primary" type="submit"
                                            id="msgSendBtn">{{ translate('send_Reply') }}</button>
                                    </div>
                                </div>
                            </form>
                        @endforeach
