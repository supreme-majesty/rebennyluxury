                        ...
                        @foreach($languages as $language)
                            <li class="nav-item text-capitalize">
                                <a class="nav-link form-system-language-tab {{ $language == $defaultLanguage? 'active':''}}" href="#"
                                   id="{{ getLanguageCode($language) }}-link">{{getLanguageName($language).'('.strtoupper($language).')'}}</a>
                            </li>
                        @endforeach
                    </ul>
                </div>
                        ...
                        <div class="{{ $language != 'en'? 'd-none' : '' }} form-system-language-form" id="{{ getLanguageCode($language) }}-form">
                            ...
                            <div class="form-group pt-4">
                                ...
                                <div class="outline-wrapper" id="editor-container-{{ getLanguageCode($language) }}">
                                      <textarea name="description[]" class="summernote" id="description-{{ getLanguageCode($language) }}"
                ...
                if (in_array_image === -1) {
                    let html = `<div class='col-6 col-md-4 col-xl-4'>
                                    <div class="position-relative p-2 border-dashed-2">
                                        ...
                                        <i class="tio-edit"></i>
                                            <input type="file" name="` + input_image_name + `" id="` + value_id + `"data-max-size="{{ getFileUploadMaxSize() }}"  class="d-none custom-upload-input-file action-upload-color-image" accept=".jpg, .webp, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*" required="">
                                        </span>
