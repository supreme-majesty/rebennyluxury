...
if (!function_exists('getFileUploadFormats')) {
    function getFileUploadFormats(string $type = "image", string|array|null $skip = null, bool $asRule = false, bool $asMessage = false): string
    {
        $formats = [
        ...
        if (!empty($skip)) {
            $skip = is_array($skip) ? $skip : explode(',', $skip);
...
if (!function_exists('getFileUploadMaxSize')) {
    function getFileUploadMaxSize(?string $type = "image"): int
    {
        $uploadMaxFileSize = (int)(checkServerUploadMaxFileSizeInMB(maxLimit: 1024) / 1024);
        if (env('APP_MODE', 'dev') === 'demo') {
            return min($uploadMaxFileSize, 2);
        }
        if (is_null($type) || $type === 'image') {
            return min($uploadMaxFileSize, 20);
        } else if ($type === 'file') {
            return min($uploadMaxFileSize, 50);
        }
        return min($uploadMaxFileSize, 20);
    }
}
