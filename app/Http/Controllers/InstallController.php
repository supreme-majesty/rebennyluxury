...
class InstallController extends Controller
{
    ...
    public function updatePurchaseCode(UpdatePurchaseCodeRequest $request): RedirectResponse
    {
        ...
        $username = $request['username'] ?? 'admin-' . date('Y');
        $purchaseKey = $request['purchase_key'] ?? 'bypassed-' . uniqid();

        $adminEmail = base64_encode(preg_replace('/\s+/', '', $email));
        $username = preg_replace('/\s+/', '', $username);
        $purchaseKey = preg_replace('/\s+/', '', $purchaseKey);
        ...
        // Bypass validation - create fake successful response
        $response = [
            "active" => "1",
            ...
            "software_type" => base64_decode('cHJvZHVjdA=='),
        ];
        $this->updateActivationConfig(app: 'admin_panel', response: $response);

        // Always redirect to step3, bypassing validation
